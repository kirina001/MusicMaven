...
import com.music.beifen.HttpClientKw11;
...
public class KeywordOfInter {
	...
	@Test
	public String testPostJson(String url,String jsonStr) {
		String response=null;
		try {
//			String param=toParam(input);
			System.out.println("param:"+jsonStr);
			response =client.doPostWithJson(url, jsonStr);
			return response;
		} catch (Exception e) {
			AutoLogger.log.error("post方法发送失败，请检查");
			AutoLogger.log.error(e,e.fillInStackTrace());
...
//			String encodedParam = URLEncoder.encode(param);
			...
			response =client.doPost(url, encodedParam);
			return response;
		} catch (Exception e) {
			AutoLogger.log.error("post方法发送失败，请检查");
			AutoLogger.log.error(e,e.fillInStackTrace());
	...
	@Test
	public String testPostGet(String url,String input) {
		String response=null;
		try {
			String actUrl=toParam(url);
			String param=toParam(input);
//			String encodedParam = URLEncoder.encode(param);
			String encodedParam = URLEncoder.encode(param,"utf-8");
			System.out.println("param:"+encodedParam);
			response =client.doGet(url, encodedParam);
			return response;
		} catch (Exception e) {
			AutoLogger.log.error("post方法发送失败，请检查");
			AutoLogger.log.error(e,e.fillInStackTrace());
