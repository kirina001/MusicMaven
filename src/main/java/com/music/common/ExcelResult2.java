...
public class ExcelResult2 {
	...
	public List<Map<String, String>> Sumarry(String path, String StartTime) {
		try {
			 ...
			 */
			System.out.println("sheet总数 "+excel.getTotalSheetNo());
...
//				System.out.println(line);
					try {
						...
						if (!((line.get(0) == null || line.get(0).length() < 2 || line.get(0).equals("null"))
								//////////bug
								&& (line.get(1) == null || line.get(1).length() < 2 || line.get(1).equals("null")))) {
						System.out.println(line + "非用例");
							// 如果第一列（分组信息）不为空则可以判断该行为分组信息行
							if (!(line.get(0) == null || line.get(0).length() < 2)) {
								// 由于第一个分组信息行是第2行，如果行数大于2，则表示读取到了第二个分组信息行，之前的用例执行结束
								if (row > 2) {
									// row>2判断开始
									// 如果flg依然为真，则状态为通过
									if (flag) {
								 ...
								 */
								name = excel.getSheetName(sheetNo) + "->" + line.get(0);
							}
						...
						// 第一列信息和第二列信息均为空的行，即用例行，这时开始进行用例数、通过数、状态的统计。
							...
							if (line.get(11) == null || line.get(11).length() < 2) {
								flag = false;
							}
							// 执行结果不为空，则将用例统计数自增
							else {
...
//							System.out.println(line);
								// 如果通过，则通过数和总通过数均自增
								if (line.get(11).equals("PASS")) {//修改前10
									totalpass++;
									detailpass++;
								} else
			...
			if (totalcount > 0) {
				...
				System.out.println("用例总数"+totalcount);
				System.out.println("通过用例总数"+totalpass);
				System.out.println("用例详情"+detailcount);
