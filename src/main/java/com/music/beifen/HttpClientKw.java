...
//		}
		...
		try {
			...
			if (jsonParam.length() > 0) {//对参数进行处理
				...
				//必填字段为空校验
//				if (jsonObject.has("musicId") && jsonObject.has("filePath")) {
//					if(jsonObject.getString("musicId").equals("")) {//id空
//						urlWithParam = url;
//						System.out.println("满足条件1"+urlWithParam);
//					}
//					if(jsonObject.getString("filePath").equals("")) {//路径空
//						musicId = jsonObject.getString("musicId");
//						urlWithParam = url + "/" + musicId;
//						System.out.println("满足条件2"+urlWithParam);
//					}
//				}
//				else {
					
//					System.out.println("满足条件3"+urlWithParam);
//				}
//				
			} 
			else {
				urlWithParam = url;
				System.out.println("满足条件4"+urlWithParam);
			}
